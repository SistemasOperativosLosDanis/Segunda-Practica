...
static int my_unlink(const char * file){
	...
	char* fileName = (char*)(file + 1); 

	//Buscamos si el archivo está en el directorio (no puedes borrar algo que no está :3)
	int idxDir = findFileByName(&myFileSystem, fileName);

	//Comprobamos si está
	if (idxDir == -1) {
		//Printf del fuse
		fprintf(stderr, "El archivo no estaba en este directorio\n");
		return -1;
	}
	
	//Sabemos que el archivo SÍ que está en el directorio
	
	myFileSystem.directory.numFiles--; //Disminuimos el número de archivos del repositorio
	myFileSystem.directory.files[idxDir].freeFile = true; //Libreramos el nodo
	updateDirectory(&myFileSystem); //Los cambios tendrán efecto en el siguiente tick

	//Recuperamos el iNodo que queremos borrar
	int nodoIdx = myFileSystem.directory.files[idxDir].nodeIdx;
	NodeStruct* nodoI = myFileSystem.nodes[nodoIdx];

	//Indicamos que hay un nuevo nodo para liberar y ponemos todos sus bloques del bitmap a 0
	myFileSystem.numFreeNodes++;
	
	for (int i = 0; i < nodoI->numBlocks; i++)
	myFileSystem.bitMap[nodoI->blocks[i]] = 0;

	//Actualizamos el superBloque con la información de los bloques eliminados y el bitmap que modificamos en el for anterior
	myFileSystem.superBlock.numOfFreeBlocks += nodoI->numBlocks;
	updateBitmap( &myFileSystem);
	updateSuperBlock( &myFileSystem);

	//Limpiamos el iNodo que hemos usado y lo liberamos
	nodoI-> numBlocks = 0;
	nodoI-> fileSize = 0;
	nodoI-> freeNode = true;
